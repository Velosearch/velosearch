...
use crate::boolean_and;
use crate::boolean_or;
...
impl BooleanQuery {
    /// Create a new boolean query expression
...
impl Expr {
    ...
    pub fn variant_name(&self) -> &str {
        match self {
            ...
            Expr::BooleanQuery(..) => "BooleanQuery",
        }
    }
    ...
    /// Return
    pub fn boolean_and(self, other: Expr) -> Expr {
        boolean_and(self, other)
    }

    /// Return
    pub fn boolean_or(self, other: Expr) -> Expr {
        boolean_or(self, other)
    }
...
impl fmt::Debug for Expr {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        match self {
            ...
            Expr::BooleanQuery(expr) => write!(f, "{expr}"),
            Expr::Sort(Sort {
                expr,
                asc,
                nulls_first,
...
fn create_name(e: &Expr) -> Result<String> {
    match e {
        ...
        Expr::BooleanQuery(boolean_query) => {
            let left = create_name(boolean_query.left.as_ref())?;
            let right = create_name(boolean_query.right.as_ref())?;
            Ok(format!("{} {} {}", left, boolean_query.op, right))
        }
        Expr::Like(Like {
            negated,
            expr,
