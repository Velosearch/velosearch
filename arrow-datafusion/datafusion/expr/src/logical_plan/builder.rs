...
use arrow::datatypes::{DataType, Schema, SchemaRef, Field};
...
pub fn project(
    ...
    debug!("Start collect project col");
    for e in expr {
        let e = e.into();
        match e {
            Expr::Wildcard => {
    ...
    debug!("End collect project col");
    // debug!("Start validate unique name");
    // validate_unique_names("Projections", projected_expr.iter())?;
    // debug!("End validate unique name");
    // let input_schema = DFSchema::new_with_metadata(
    //     exprlist_to_fields(&projected_expr, &plan)?,
    //     plan.schema().metadata().clone(),
    // )?;
    let input_schema = DFSchema {
        fields: exprlist_to_fields(&projected_expr, &plan)?,
        metadata: plan.schema().metadata().clone(),
    };
    debug!("End construct project");
    Ok(LogicalPlan::Projection(Projection::try_new_with_schema(
        projected_expr,
        Arc::new(plan.clone()),
