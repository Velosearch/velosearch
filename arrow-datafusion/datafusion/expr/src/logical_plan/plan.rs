...
impl LogicalPlan {
    ...
    pub fn schema(&self) -> &DFSchemaRef {
        match self {
            ...
            LogicalPlan::Boolean(Boolean { input, ..}) => input.schema(),
        ...
        F: FnMut(&Expr) -> Result<(), E>,
    {
        match self {
            ...
            LogicalPlan::Boolean(Boolean { binary_expr, ..}) => f(binary_expr),
    ...
    pub fn inputs(self: &LogicalPlan) -> Vec<&LogicalPlan> {
        match self {
            ...
            LogicalPlan::Boolean(Boolean { input, ..}) => vec![input],
...
impl LogicalPlan {
        ...
        V: PlanVisitor,
    {
        ...
        let recurse = match self {
            ...
            LogicalPlan::Boolean(Boolean { input, ..}) => input.accept(visitor)?,
...
pub enum Predicate {
    /// Predicate, Selectivity, Node num, Leaf num
    And { args: Vec<(Predicate, f64, usize, usize)> },
    /// Predicate, Selectivity, None num, Leaf num
    Or { args: Vec<(Predicate, f64, usize, usize)> },
    /// Leaf
    Other { expr: Box<Expr> },
}
...
pub struct Boolean {
    /// The predicate expression, which must have Boolean type.
    pub binary_expr: Expr,
    /// The simplified predicate after logical rewritten.
    pub predicate: Option<Predicate>,
    /// The number of expr in predicate
    pub expr_cnt: usize,
    /// The number of operator cnt in predicate
    pub op_cnt: usize,
    /// is_score
    pub is_score: bool,
    /// The incoming logical plan
