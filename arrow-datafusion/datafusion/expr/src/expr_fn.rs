...
use crate::expr::{AggregateFunction, BinaryExpr, Cast, GroupingSet, TryCast, BooleanQuery};
use crate::{
...
/// Return a boolean_query expression
pub fn boolean_query_expr(left: Expr, op: Operator, right: Expr) -> Expr {
    Expr::BooleanQuery(BooleanQuery::new(Box::new(left), op, Box::new(right)))
}

/// Return a new boolean expression with AND op
pub fn boolean_and(left: Expr, right: Expr) -> Expr {
    boolean_query_expr(left, Operator::BitwiseAnd, right)
}

/// Return a new boolean expression with OR op
pub fn boolean_or(left: Expr, right: Expr) -> Expr {
    boolean_query_expr(left, Operator::BitwiseOr, right)
}

/// Return a new boolean expression with XOR op
pub fn boolean_xor(left: Expr, right: Expr) -> Expr {
    boolean_query_expr(left, Operator::BitwiseXor, right)
}
