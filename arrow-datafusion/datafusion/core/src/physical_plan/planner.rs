...
use datafusion_expr::expr::{
    ...
    Like, TryCast, WindowFunction, BooleanQuery,
};
        ...
        async move {
            let exec_plan: Result<Arc<dyn ExecutionPlan>> = match logical_plan {
                ...
                LogicalPlan::Boolean(boolean) => {
                    let physical_input = self.create_initial_plan(&boolean.input, session_state).await?;
                    let input_schema = physical_input.as_ref().schema();
                    let input_dfschema = boolean.input.schema();

                    let _runtime_expr = self.create_physical_expr(
                        &boolean.binary_expr,
                        input_dfschema,
                        &input_schema,
                        session_state
                    ...
                    let _num_partition = physical_input.output_partitioning().partition_count();
                    unimplemented!();
                }
