...
impl RoaringBitmap {
    ...
    /// Returns the memory consumpltion
    pub fn memory_consumption(&self) -> usize {
        self.containers.iter().map(|c| c.memory_consumption()).sum()
    }
    ...
    /// Return the `RankIter` to optimize the ranke operations.
    pub fn rank_iter(&self) -> RankIter {
        RankIter::new(&self)
    }
