...
impl BitmapStore {
    ...
    pub fn rank_base(&self, base: &mut usize, index: u16) -> usize {
        let (a_key, a_bit) = (key(index), bit(index));

        if *base == usize::MAX {
            *base = index as usize; 
            return self.bits[..a_key].iter().map(|v| v.count_ones() as usize).sum::<usize>()
            + (self.bits[a_key] << (63 - a_bit)).count_ones() as usize;
        }

        let (b_key, b_bit)  = (key(*base as u16), bit(*base as u16));
        // println!("base: {:}, index: {:}", base, index);
        *base = index as usize;
        if a_key == b_key {
            let rank = ((self.bits[a_key] << (63 - a_bit)).count_ones() - (self.bits[b_key] << (63 - b_bit)).count_ones()) as usize;
            // println!("{:}", rank);
            // println!("{:}, {:}", (self.bits[a_key] << (63 - a_bit)).count_ones(), (self.bits[b_key] << (63 - b_bit)).count_ones());
            rank
        } else {
            let rank = self.bits[b_key..a_key].iter().map(|v| v.count_ones() as usize).sum::<usize>() + (self.bits[a_key] << (63 - a_bit)).count_ones() as usize - (self.bits[b_key] << (63 - b_bit)).count_ones() as usize;
            // println!("else {:}", rank);
            rank
        }
    }
